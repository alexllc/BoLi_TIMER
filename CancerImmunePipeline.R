...
## Bug fixed by Alexandria Lau

## manually load your cancer.RData by clicking the file
cc <- 'hnsc'  # we set cancer name here as hnsc for demonstration
cur.dir=""
source('CancerImmunePipeline.R', local = TRUE)

##----- setup parameters and establish the output file -----##
if(cc=='skcm')cc.type='06A' else cc.type='01A'
cur.dir='/home/satan/Documents/VL/TIMER/'  ## Change this to your own analysis directory
...
dd <- as.data.frame(hnscnn)
sel <- hnscnn[,(hnscnn[1,]) == "scaled_estimate"]
row.names(sel) = dd$Hybridization.REF  #make first column rownames

  #dd[,(dd[1,]) == "scaled_estimate"] #only select the columns that contain the scaled estimates(to calculate TPM later)
dd <- sel
  #dd$Hybridization.REF = NULL  #remove the first column
  #dd = as.data.frame(dd)[-1,]
#if(cc %in% c('gbm','ov')){
#  dd=load(file = 'hnsc.RData', envir = '/home/satan/Documents/VL/TIMER/')
#}else dd=get(load(paste(cur.dir,'data/RNAseq/',cc,'RNAseq.Rdata',sep='')))
dd=as.matrix(dd)
tmp=strsplit(rownames(dd),'\\|') #needs to be done before converting into numeric, unless you formatted the gene ids into rownames
mode(dd)='numeric'
...
tmp.vv=which(nchar(tmp)>1)  ##select the entries that actually has a gene name
rownames(dd)=tmp
dd=dd[tmp.vv,] #by now you should have alphabets rownames and numeric cells
dd <- dd[-1,] #remove the gene_id row
...
##Run these lines manually, the original ones included too many negative values
  library(sva)
  tmp.dd=as.matrix(dd)
  tmp=sapply(strsplit(as.character(rownames(dd)),'\\|'),function(x)x[[1]])
  rownames(tmp.dd)=tmp
  ...
  mode(tmp.dd)='numeric'
  tmp.dd = (tmp.dd[(rowVars(tmp.dd)) > 0,])  #remove 0 variance
  N2=ncol(curated.ref.genes)
  tmp.batch=c(rep(1,N1),rep(2,N2))
  tmp.dd0=ComBat(tmp.dd,tmp.batch,c())
  tmp.dd0 = ifelse(tmp.dd0<0,0,tmp.dd0)   #change the negative values to 0
  dd.br=tmp.dd0[,1:N1]
...
getPurityGenes <- function(dd,AGP,thr.p=0.05,thr.c=0,mode='env'){
  ...
  tmp.dd=dd[,tmp.ss]
  tmp=lapply(rownames(tmp.dd),function(x)cor.test(tmp.dd[x,],AGP[colnames(tmp.dd),2],method='s'))
...
##----- CD4 and CD8 T cells are likely to be similar, resulting in colinearity. 
##----- Codes below are procedures to remove outlier genes that may result in colinearity 
##----- until the two covariates are linearly separable. -----##
...
##----- Simulation based evaluation of inference based on 
##----- 1) gene set vv and 2) gene-gene correlation structure of the cancer data -----##
library(MASS)
thr.sd=1
N=nrow(Fmat)
#N=1
...
load('/home/satan/Documents/VL/TIMER/Redblue.Rdata')  #or do this manually
redblue <- as.matrix(redblue)  ##must convert the rb data into numeric
heatmap(cor(Fmat),col=redblue)

pred <- hist(cor(Fmat)[upper.tri(cor(Fmat))]) ## negative correlations are driven by co-linearity. Positive correlations are likely to be true.
...
if('T_cell.CD8' %in% colnames(Fmat)){
  ...
  cat('CD8B\n\n',file='output-statistics.txt',append=T)
  dd.br[dd.br == 0] <- NA  ## NEED TO REPLACE 0 WITH NA FOR THE LOG VALUES TO BE CALCULATED
...
necrosisData=read.table(paste(cur.dir,'/data/Clinical/clinical_',cc,'.tsv',sep=''),sep='\t',header=T)
...
## Survival Analysis
library(survival)
dd.surv=read.table(paste(cur.dir,'data/Clinical/clinical_',cc,'.tsv',sep=''),header=T,sep='\t',stringsAsFactors=F)
